...
- **Workbench** — model/schema/router, `POST /api/workbench/{id}/resolve` flow, idempotency guard against re-resolving a terminal item. **New:** `POST /api/workbench` (generic create) lets an Operator/Orchestrator raise an exception directly — `missing_data`, `low_confidence`, `high_stakes`, `novel_scenario` — not just the automatic `policy_conflict` path below. Uses the `WorkItemCreate` schema that already existed in `app/schemas/work_item.py` but was unwired until now.
- **Policies** — full CRUD + activate/deactivate lifecycle, plus structured-only evaluation at `POST /api/ai/policies/evaluate`. A conflict (permissive + gating policy both match) auto-creates a `WorkItem` (`exception_type=POLICY_CONFLICT`). **New:** four procurement policies are now seeded active by `scripts/seed_db.py` (idempotent, keyed by name) — Expedite Spend Limit (>RM50k → `require_approval`), Contract Escalation Clause Block (`requires_vp_signoff` → `block`), Substitution Eligibility (MOQ/quote-expiry → `flag_for_review`), Customer-Tier Priority Escalation (critical customer → `escalate`). Before this, **zero policies were ever seeded** — earlier notes in this file implying "3 active policies" existed referred only to engine capability proven in tests, not live data; verify that distinction before trusting old status notes at face value.
- **Insights** — statistical pattern/anomaly/recommendation generation over recent audit-log activity via `POST /api/ai/insights/generate`. No LLM involved. **New:** a second insight family, computed by `app/services/insight_engine.py`'s `_derive_operational_insights()` reading `app/services/supabase_client.py` (a thin read-only httpx wrapper) directly against the live Supabase procurement dataset — recurring delay patterns by supplier, POs trending at risk, demand anomalies (correctly uses `on_hand_qty − committed_qty` as available stock, not raw on-hand — the dataset's phantom-inventory trap), single-source exposure, contracts expiring within 90 days. Degrades to zero extra insights (not an error) if `SUPABASE_URL`/`SUPABASE_SERVICE_KEY` aren't set in `.env`. `generate_insights()` is now `async` because of this — the router already `await`s it.
...
All four have pytest coverage in `tests/` (`test_policy_engine.py`, `test_insight_engine.py`, `test_ai_manager.py`) and were verified live against the running Docker containers, not just unit-tested. Note `scripts/` is baked into the backend image at build time, not bind-mounted like `app/`/`tests/` — after editing anything under `scripts/`, `docker compose up -d --build backend` before `docker compose exec backend python scripts/seed_db.py` will otherwise silently run the stale copy.

**Dataset handling:** `Operations_Case_Study/` (the case-study PDFs + CSVs) is intentionally left untracked/uncommitted — the Round 2 guide states the dataset "may not be redistributed," and this repo's `origin` remote is a public-facing GitHub repo judges will be given. Seed it into Supabase locally per the plan in `NEXT_STEPS.md`; don't `git add` the folder itself.
...
`app/services/ai_manager.py`'s private `_dispatch(...)` function is the intended seam. Once the token resets, that's where a real Auto API call replaces (or sits alongside) the direct `policy_engine.evaluate(...)` call — `handle_chat_message`'s public signature and the router don't need to change. The Round 1 Auto workflow exports under `Operations_Case_Study/Round 1 Operator/` (Master Orchestrator, Impact Mapper, Alternative Sourcer) are the existing pattern to extend to 5+ Operators — see `NEXT_STEPS.md`.
